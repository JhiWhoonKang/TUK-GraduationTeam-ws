...
namespace RCWS_Situation_room
{
    public partial class FormMain : Form
    {
        ...
        public FormMain(StreamWriter streamWriter, FormDataSetting formDataSetting)
        {
            ...
            _UDPService= new UDPService(this);
            _UDPService.VideoReceived += OnVideoReceived;
        ...
        private void Joy_Run()
        {
            ...
            while (true)
            {
                ...
                {
                    byte[] commandBytes = TCPCmd.StructToBytes(SEND_DATA);
        ...
        {
            var serverIp = Define.SERVER_IP;
            var port = Define.TCPPORT;
        ...
        {
            await _tcpService.ConnectAsync(serverIp, port);         
        }
        ...
        private void HandleReceivedData(byte[] data)
        {
            if (InvokeRequired)
            {
                Invoke(new Action<byte[]>(HandleReceivedData), data);
                return;
            }

            RTB_RECEIVED_DISPLAY.AppendText(BitConverter.ToString(data) + "\r\n");
            var receivedData = TCPCmd.BytesToStruct<Packet.RECEIVED_PACKET>(data);
        ...
        private async Task ReceiveDataAsync()
        {
            try
            {
                while (true)
                {
                    ...
                    RECEIVED_DATA = TCPCmd.BytesToStruct<Packet.RECEIVED_PACKET>(receivedData);
        ...
        {            
            //Task.Run(() => UdpConnect());
            Task.Run(() => _UDPService.InitializeReceiving());
        }        
